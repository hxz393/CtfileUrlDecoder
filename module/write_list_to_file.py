...
`write_list_to_file` 函数的主要目标是将列表的元素写入文件，每个元素占据文件的一行。该函数接受两个参数：`target_path` 和 `content`。 `target_path` 参数是将要写入的文本文件的路径，可以是字符串或 `pathlib.Path` 对象。 `content` 参数是要写入的列表。
